...
@interface EMChatTextBubbleView ()<TTTAttributedLabelDelegate, UIActionSheetDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _textLabel.highlightedTextColor = [UIColor greenColor];
        _textLabel.enabledTextCheckingTypes = NSTextCheckingTypeLink;
          _textLabel.linkAttributes = [NSDictionary dictionaryWithObject:[NSNumber numberWithBool:NO] forKey:(NSString *)kCTUnderlineStyleAttributeName];
...
//    _strrr = [self filterHTML:self.model.content];
...
- (void)setModel:(MessageModel *)model
{
    ...
    NSMutableAttributedString * attributedString = [[NSMutableAttributedString alloc]
                                                    initWithString:_strrr];
    NSMutableParagraphStyle * paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    [paragraphStyle setLineSpacing:[[self class] lineSpacing]];
    [attributedString addAttribute:NSParagraphStyleAttributeName
                             value:paragraphStyle
                             range:NSMakeRange(0, [_strrr length])];
    if (self.model.isSender) {
        [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, [_strrr length])];
    }
    else {
        [attributedString addAttribute:NSForegroundColorAttributeName value:CCCUIColorFromHex(0x333333) range:NSMakeRange(0, [_strrr length])];
    }
    
    [attributedString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15] range:NSMakeRange(0, [_strrr length])];
...
//     NSString *string2 = [string1 substringFromIndex:3];
    ...
    for (NSTextCheckingResult *match in _urlMatches) {
        ...
        if ([match resultType] == NSTextCheckingTypeLink || [match resultType] == NSTextCheckingTypeReplacement) {
            ...
            NSRange rangefen = [_strrr rangeOfString:@"【"];
            
            
            
            NSString *str = [_strrr substringFromIndex:rangefen.location];
            
            NSString *str2 = [_strrr substringToIndex:rangefen.location];
            
            NSLog(@"%@",str);
//            NSRange matchRange = [match range];
            NSRange matchRange = [_strrr rangeOfString:str];
            
            
            NSRange matchRange2 = [_strrr rangeOfString:str2];
            
             [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor lightGrayColor] range:matchRange2];
            
            if ([self isIndex:index inRange:matchRange]) {
                [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor lightGrayColor] range:matchRange];
            }
            else {
