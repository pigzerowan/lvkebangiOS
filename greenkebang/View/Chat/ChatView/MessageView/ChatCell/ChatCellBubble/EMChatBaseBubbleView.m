...
{
    _model = model;
    NSLog(@"----------%@是否分享-------",model.content);
    
    if([model.content rangeOfString:@"shareType"].location !=NSNotFound||[model.content rangeOfString:@".jpg"].location !=NSNotFound)//_roaldSearchText
    {

        BOOL isReceiver = !_model.isSender;
        NSString *imageName = isReceiver ? BUBBLE_LEFT_IMAGE_NAMESHARE : BUBBLE_RIGHT_IMAGE_NAMESHARE;
