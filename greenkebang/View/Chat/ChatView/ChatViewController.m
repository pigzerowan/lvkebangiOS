...
//    UIView *contentView;
    if ( [[self.tabBarController.view.subviews objectAtIndex:0] isKindOfClass:[UITabBar class]] )
        self.view = [self.tabBarController.view.subviews objectAtIndex:1];
    else
         self.view = [self.tabBarController.view.subviews objectAtIndex:0];
     self.view.frame = CGRectMake( self.view.bounds.origin.x,   self.view.bounds.origin.y,   self.view.bounds.size.width,  self.view.bounds.size.height + self.tabBarController.tabBar.frame.size.height);
...
//    self.view.backgroundColor = UIColorWithRGBA(248, 248, 248, 1);
...
- (void)setupBarButtonItem
{
    ...
    if (self.isChatGroup) {
        UIButton *friendBtn = [[UIButton alloc]initWithFrame:CGRectMake(286, 12, 24, 24)];
        [friendBtn setImage:[UIImage imageNamed:@"addright"] forState:UIControlStateNormal];
//        [friendBtn addTarget:self action:@selector(toHandeMyFriend:) forControlEvents:UIControlEventTouchUpInside];
...
- (UITableView *)tableView
{
    if (_tableView == nil) {
        ...
        _tableView.backgroundColor = CCCUIColorFromHex(0xf0f1f2);
...
- (DXMessageToolBar *)chatToolBar
{
    if (_chatToolBar == nil) {
        ...
        _chatToolBar.moreView.backgroundColor = UIColorWithRGBA(240, 242, 247, 1);
...
{
    if ([eventName isEqualToString:LalalaShareName]) {
        [self chatShareCellUrlPressed:userInfo];
    }
    else
    {
...
- (void)chatHeadImagePressed:(MessageModel *)model
{
    ...
    [ToUserManager shareInstance].userId = model.username;
    NewUserMainPageViewController *peopleVC = [[NewUserMainPageViewController alloc] init];
    
    peopleVC.type = @"2";
    peopleVC.toUserId = model.username;
    
    peopleVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:peopleVC animated:YES];

//    UserRootViewController *sss = [[UserRootViewController alloc]init];
//    sss.type  = @"2";
//    sss.toUserId = model.username;
...
//    [appDelegate showUserBaseTabBarViewController];

//    AppDelegate *appDelegate =  (AppDelegate*)[UIApplication sharedApplication].delegate;
//    if ([AppDelegate shareInstance].lkbUserbaseVc ) {
//        self.view.window.rootViewController = [AppDelegate shareInstance].lkbUserbaseVc;
//    }else
//    {
//        [appDelegate showUserBaseTabBarViewController];
//    }
...
//分享被点击
- (void)chatShareCellUrlPressed:(NSDictionary *)sharedic
{
    ...
    NSString *urlStr = sharedic[@"message"][@"linkUrl"];

    
    NSString *shareType =   [NSString stringWithFormat:@"%@",sharedic[@"message"][@"shareType"]] ;
    NSString *circleId = sharedic[@"message"][@"circleId"];// 圈Id
    NSString *featureId = sharedic[@"message"][@"featureId"];// 圈详情Id
    NSString *title = sharedic[@"message"][@"title"];
    NSString *toUserId = sharedic[@"message"][@"toUserId"];
    NSString *columnId = sharedic[@"message"][@"columnId"]; // 专栏Id
    NSString *str = sharedic[@"message"][@"columnAvatar"]; // 专栏头像
    NSString * columnAvatar = [str lastPathComponent]; //截取字符串
    NSString *replyNum = sharedic[@"message"][@"replyNum"]; // 专栏头像
    ...
    if ([shareType isEqualToString:@"2"]) {
        ...
        NSString *linkUrl;
        if (!circleId ) {
              linkUrl = [NSString stringWithFormat:@"%@",urlStr];
        }
        
        else
        {
          linkUrl = [NSString stringWithFormat:@"%@/detail/agriculture/%@?sourceId=%@",LKB_WSSERVICE_HTTP,featureId, circleId];
        }
        ...
        outSideWeb.circleId = circleId;
        outSideWeb.circleDetailId = featureId;
...
//        outSideWeb.isAttention = insight.isAttention;
    ...
    if ([shareType isEqualToString:@"3"]) {
        // 活动详情
        NSString *linkUrl = [NSString stringWithFormat:@"%@/detail/activity/%@",LKB_WSSERVICE_HTTP,featureId];
        NSString *strmy = [linkUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
//        outSideWeb.columnAvatar = attentionModel.img;
    ...
    if ([shareType isEqualToString:@"4"]) {
        // 专栏详情
        NSString *linkUrl = [NSString stringWithFormat:@"%@/detail/insight/%@",LKB_WSSERVICE_HTTP,featureId] ;
        NSString *strmy = [linkUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        ...
        outSideWeb.rightButtonType = @"2";
        outSideWeb.VcType = @"2";
        outSideWeb.objectId = featureId;
        outSideWeb.circleDetailId = featureId;
        outSideWeb.mytitle = title;
        outSideWeb.featureId = columnId; // 专栏Id
        outSideWeb.couAvatar = columnAvatar;
        outSideWeb.replyNum = replyNum;
        //        outSideWeb.mytitle = @"";
        //        outSideWeb.describle = insight.summary;
        //        outSideWeb.userAvatar = insight.userAvatar;
        //        outSideWeb.isAttention = insight.isAttention;
    ...
    if ([shareType isEqualToString:@"5"]) {
        // 锄禾说
        NSString *linkUrl = [NSString stringWithFormat:@"%@/detail/hoeContent/%@",LKB_WSSERVICE_HTTP,featureId];
        NSString *strmy = [linkUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    if ([shareType isEqualToString:@"7"]) {
        
        NewUserMainPageViewController *peopleVC = [[NewUserMainPageViewController alloc] init];
        
        peopleVC.type = @"2";
        peopleVC.toUserId = toUserId;
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop)
     {
         if ([obj isKindOfClass:[MessageModel class]])
         {
             ...
             {
                 //自己个人头像
                 model.headImageURL =[[MyUserInfoManager shareInstance].avatar lkbImageUrl4];
...
//                 model.headImageURL =[[MyUserInfoManager shareInstance].avatar lkbImageUrl];
                 *stop = YES;
             }
         }
     }];
...
//                            cellModel.headImageURL = cellModel.message.ext[]
...
//                model.nickName = showName?showName:model.username;
                NSLog(@"ext的值是什么=====%@",model.message.ext);
...
//                model.nickName = model.username;
            ...
            if ([_delelgate respondsToSelector:@selector(avatarWithChatter:)]) {
                ...
                model.headImageURL = [model.message.ext[@"userAvatar"] lkbImageUrl4];
...
//        model.nickName = model.username;
    ...
    if ([_delelgate respondsToSelector:@selector(avatarWithChatter:)]) {
         NSLog(@"======%@lalallalal=====",model.username);
...
-(void)sendTextMessage:(NSString *)textMessage
{
    ...
    NSDictionary *ext = @{@"userAvatar":[[MyUserInfoManager shareInstance]avatar],
                          @"userName":[[MyUserInfoManager shareInstance]userName],
...
-(void)sendShareMessage:(NSString *)shareModle withIfShare:(NSString *)sharetype
{
    
    NSLog(@"===%@=========",[[MyUserInfoManager shareInstance]avatar]);
    
     NSLog(@"===%@=ssss========",[[MyUserInfoManager shareInstance]userName]);
     NSLog(@"===%@=ssss========",[[MyUserInfoManager shareInstance]userId]);
    ...
    //NSDictionary *ext = nil;
    EMMessage *tempMessage = [ChatSendHelper sendTextMessageWithString:shareModle
                                                            toUsername:_conversation.chatter
                                                           messageType:[self messageType]
                                                     requireEncryption:NO
                                                                   ext:ext];
