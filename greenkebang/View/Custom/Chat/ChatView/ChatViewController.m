...
#import "ShowGroupTopicViewController.h"
#import "ShowGroupQAViewController.h"
...
- (UITableView *)tableView
{
    if (_tableView == nil) {
        ...
        _tableView.backgroundColor = UIColorWithRGBA(248, 248, 248, 1);
...
//                model.nickName = model.username;
            ...
            if ([_delelgate respondsToSelector:@selector(avatarWithChatter:)]) {
                ...
                model.headImageURL = [model.message.ext[@"userAvatar"] lkbImageUrl];
...
-(void)toHandeMyFriend:(UIButton *)sender
{
    CGPoint point = CGPointMake(sender.frame.origin.x + sender.frame.size.width/2, sender.frame.origin.y + sender.frame.size.height);
    NSArray *titles = @[@"查看话题", @"查看问答", @"群组详情"];
    NSArray *imgArrays = @[@"sender_talk",@"creatgroup",@"saoma"];
    PopoverView *pop = [[PopoverView alloc] initWithPoint:point titles:titles images:imgArrays];
    pop.selectRowAtIndex = ^(NSInteger index){
        NSLog(@"select index:%ld", (long)index);
        NSUserDefaults *userDefualt = [NSUserDefaults standardUserDefaults];
        NSDictionary *dic = [userDefualt objectForKey:_chatter];
        if (index==1) {
            self.hidesBottomBarWhenPushed = YES;
            ShowGroupQAViewController *creatGroupVC = [[ShowGroupQAViewController alloc] init];
            creatGroupVC.groupId = dic[@"groupId"];
        ...
        if (index==0) {
             self.hidesBottomBarWhenPushed = YES;
            ShowGroupTopicViewController *contactsGroupVC = [[ShowGroupTopicViewController alloc] init];
            contactsGroupVC.groupId = dic[@"groupId"];
            [self.navigationController pushViewController:contactsGroupVC animated:YES];
        }
        if (index==2) {
            self.hidesBottomBarWhenPushed = YES;
            
            GroupInfoDetailViewController *detailController = [[GroupInfoDetailViewController alloc] init];
            
           
            detailController.groupIdStr = dic[@"groupId"];
...
-(void)sendTextMessage:(NSString *)textMessage
{
    NSDictionary *ext = @{@"userAvatar":[MyUserInfoManager shareInstance].avatar ,
                          @"userName":[MyUserInfoManager shareInstance].nickName,
