...
- (UIImageView *)arrow {
    if(!_arrow) {
        //        CGFloat arrowX = (self.bounds.size.width / 2) - (totalMaxWidth / 2) + (leftViewWidth - self.arrow.bounds.size.width) / 2;
        _arrow = [[UIImageView alloc]initWithFrame:CGRectMake((self.bounds.size.width-18)/2, (self.bounds.size.height-25)/2, 27, 27)];
        _arrow.image = [UIImage imageNamed:@"箭头.png"];
        //greenLoading.gif
//        _arrow.image = [UIImage imageNamed:@"greenLoading.gif"];
...
//        _titleLabel.text = NSLocalizedString(@"绿科邦正在加载...",@"Pull to refresh...");
