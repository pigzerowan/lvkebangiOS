...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.titleLabel.textColor = CCCUIColorFromHex(0x888888);
...
- (void)setSelected:(BOOL)selected {
    _selected = selected;
    _titleLabel.textColor = CCCUIColorFromHex(0x888888);
...
//    [bezierPath stroke];
    ...
    if (self.selected) {
        ...
        // Draw the indicator 划线 3个的情况下
//        [bezierPath moveToPoint:CGPointMake(CGRectGetHeight(rect)  /2 +35   , CGRectGetHeight(rect) -4.0) ];
//        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect) -60, CGRectGetHeight(rect) -4.0)];
        

        [bezierPath moveToPoint:CGPointMake(CGRectGetHeight(rect)  /2 +45  , CGRectGetHeight(rect) -1.0) ];
        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect) -70, CGRectGetHeight(rect) -1.0)];

        
//        [bezierPath addLineToPoint:CGPointMake(20.0,20.0)];
        [bezierPath setLineWidth:2.0];
        
        [self.indicatorColor setStroke];
        [bezierPath stroke];
        
        // change title color 21, 174, 237
        
        self.titleLabel.textColor = [UIColor LkbgreenColor];
//        [UIColor colorWithRed:21.0/255.0 green:174.0/255.0 blue:237.0/255.0 alpha:1];
    }
}
@end
...
- (void)layoutSubviews {
    ...
    self.searchButton.frame = CGRectMake(frame.size.width-22, frame.origin.y+1, 22, frame.size.height-2);
...
{
    if ([self.delegate respondsToSelector:@selector(viewPagerDidStartSearch:)]) {
        [self.delegate viewPagerDidStartSearch:self];
    }
}
...
//    self.searchButton = (UIButton *)[self.view viewWithTag:kSearchButtonTag];
//    if (!self.searchButton) {
//        self.searchButton = [UIButton buttonWithType:UIButtonTypeCustom];
//        self.searchButton.backgroundColor = [UIColor whiteColor];
//        self.searchButton.frame = CGRectMake(CGRectGetWidth(self.view.frame)-22, 0, 22, [self.tabHeight floatValue]);
//        self.searchButton.tag = kSearchButtonTag;
//        [self.searchButton setImage:[UIImage imageNamed:@"yq_goods_search_icon.png"] forState:UIControlStateNormal];
//        [self.searchButton addTarget:self action:@selector(searchButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
//        [self.view insertSubview:self.searchButton atIndex:1];
//    }
