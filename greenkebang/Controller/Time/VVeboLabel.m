            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (drawFlag==flag) {
                    if (isHighlight) {
                        if (highlighting) {
                            highlightImageView.image = nil;
                            if (highlightImageView.width2!=screenShotimage.size.width) {
                                highlightImageView.width2 = screenShotimage.size.width;
                            }
                            if (highlightImageView.height2!=screenShotimage.size.height) {
                                highlightImageView.height2 = screenShotimage.size.height;
                            }
                            highlightImageView.image = screenShotimage;
                        }
                    } else {
                        if ([temp isEqualToString:text]) {
                            if (labelImageView.width2!=screenShotimage.size.width) {
                                labelImageView.width2 = screenShotimage.size.width;
                            }
                            if (labelImageView.height2!=screenShotimage.size.height) {
                                labelImageView.height2 = screenShotimage.size.height;
                            }
                            highlightImageView.image = nil;
                            labelImageView.image = nil;
        ...
        if ((!highlighting&&self.superview!=nil)) {
            ...
            for (int j = 0; j < CFArrayGetCount(runs); j++) {
                ...
                if (!CGColorEqualToColor((__bridge CGColorRef)([attributes valueForKey:@"CTForegroundColor"]), self.textColor.CGColor)
                    ...
                    runRect=CGRectMake(lineOrigin.x + offset, (self.height2+5)-y-height+runDescent/2, runRect.size.width, height);
