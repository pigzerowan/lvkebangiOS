//
//  TimeThreeImageWithCommentCell.m
//  greenkebang
//
//  Created by 徐丽霞 on 16/8/31.
...
@implementation TimeThreeImageWithCommentCell
...
- (void)configTimeThreeImageWithCommentCellTimeNewDynamicDetailModel:(NewDynamicDetailModel *)model {
...
//    [_coverListImage_2 sd_setImageWithURL:[model.coverList[2] lkbImageUrlAllCover] placeholderImage:YQNormalBackGroundPlaceImage];
    ...
    NSString *str_1 = [NSString stringWithFormat:@"%@:%@",[[model.replyList objectAtIndex:0] objectForKey:@"userName"],[[model.replyList objectAtIndex:0] objectForKey:@"content"]];
    
    titleSize_1 = [str_1 boundingRectWithSize:CGSizeMake(kDeviceWidth - 35, KDeviceHeight ) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:12]} context:nil];
    NSMutableAttributedString* priAttributedStr_1 = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@:%@",[[model.replyList objectAtIndex:0] objectForKey:@"userName"],[[model.replyList objectAtIndex:0] objectForKey:@"content"] ]];
    [priAttributedStr_1 addAttribute:NSForegroundColorAttributeName value:CCCUIColorFromHex(0x555555) range:NSMakeRange(0, str_1.length)];
    NSString *nameStr_1 = [NSString stringWithFormat:@"%@:",[[model.replyList objectAtIndex:0] objectForKey:@"userName"]];
    NSRange range_1 = [str_1 rangeOfString:nameStr_1];
    [priAttributedStr_1 addAttribute:NSForegroundColorAttributeName value:CCCUIColorFromHex(0x999999) range:range_1];
//    [_commentlabel_1 sizeToFit];
    _commentlabel_1.text = priAttributedStr_1;
    NSString *str_2 = [NSString stringWithFormat:@"%@：%@",[[model.replyList objectAtIndex:1] objectForKey:@"userName"],[[model.replyList objectAtIndex:1] objectForKey:@"content"]];
    titleSize_2 = [str_2 boundingRectWithSize:CGSizeMake(kDeviceWidth - 35, KDeviceHeight ) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:12]} context:nil];
    NSMutableAttributedString* priAttributedStr_2 = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@",[[model.replyList objectAtIndex:1] objectForKey:@"userName"],[[model.replyList objectAtIndex:1] objectForKey:@"content"] ]];
    [priAttributedStr_2 addAttribute:NSForegroundColorAttributeName value:CCCUIColorFromHex(0x555555) range:NSMakeRange(0, str_2.length)];
    NSString *nameStr_2 = [NSString stringWithFormat:@"%@：",[[model.replyList objectAtIndex:1] objectForKey:@"userName"]];
    NSRange range_2 = [str_2 rangeOfString:nameStr_2];
    [priAttributedStr_2 addAttribute:NSForegroundColorAttributeName value:CCCUIColorFromHex(0x999999) range:range_2];
//    [_commentlabel_2 sizeToFit];
    _commentlabel_2.text = priAttributedStr_2;
...
- (void)configFarmerThreeImageWithCommentCellTimeNewDynamicDetailModel:(NewDynamicDetailModel *)model {
...
//    [self.contentView addSubview:self.lineView];
    ...
    self.commentlabel_1.preferredMaxLayoutWidth = w-38 ;
    self.commentlabel_1.userInteractionEnabled = YES;
    self.commentlabel_2.preferredMaxLayoutWidth = w-38 ;
    self.commentlabel_2.userInteractionEnabled = YES;
...
- (void)updateConstraints
{
    ...
    if (!self.didSetupConstraints) {
        ...
        [_coverListImage mas_makeConstraints:^(MASConstraintMaker* make) {
            ...
            make.left.mas_equalTo(_headImageView);
            make.size.mas_equalTo(CGSizeMake(kDeviceWidth , ((kDeviceWidth -28 -6)/3)*0.95));
        }];
        ...
        [_coverListImage_2 mas_makeConstraints:^(MASConstraintMaker* make) {
            make.top.mas_equalTo(_goodsDesLabel.bottom).offset(10);
            make.left.mas_equalTo(_coverListImage_1.right).offset(3);
            make.size.mas_equalTo(CGSizeMake((kDeviceWidth -28 -6)/3, ((kDeviceWidth -28 -6)/3)*0.95));
        }];
        ...
        [_commentlabel_2 mas_makeConstraints:^(MASConstraintMaker* make) {
            
            make.top.mas_equalTo(_commentlabel_1.bottom).offset(5);
            make.left.mas_equalTo(self.CommentImageView.right).offset(10);
            make.width.mas_equalTo(self.contentView).offset(-47);
            
        }];
        
        [_commentButton_2 mas_makeConstraints:^(MASConstraintMaker* make) {
            make.top.mas_equalTo(_commentlabel_1.bottom).offset(2);
            make.left.mas_equalTo(self.CommentImageView.right).offset(5);
            make.size.mas_equalTo(CGSizeMake(45, 30));
        ...
        [_lastView mas_makeConstraints:^(MASConstraintMaker* make) {
            ...
            make.top.mas_equalTo(_commentlabel_2.mas_bottom).offset(15);
...
- (void)ThreeImageWithCommenthandlerButtonAction:(BottomBuyClickBlock)block {
...
// 图3
- (UIImageView *)coverListImage_2 {
    
    if (!_coverListImage_2) {
        _coverListImage_2 = [[UIImageView alloc]init];
//        _coverListImage_2.backgroundColor = CCCUIColorFromHex(0xdddddd);
        _coverListImage_2.clipsToBounds = YES;
        _coverListImage_2.contentMode = UIViewContentModeScaleAspectFill;
        _coverListImage_2.userInteractionEnabled = YES;

        
    }
    return _coverListImage_2;
}
...
- (TTTAttributedLabel *)commentlabel_2 {
    
    if (!_commentlabel_2) {
        
        _commentlabel_2 = [[TTTAttributedLabel alloc]initWithFrame:CGRectMake(10, titleSize_1.size.height+8, kDeviceWidth -35, 30)];
//        _commentlabel_2.backgroundColor = CCCUIColorFromHex(0xf7f7f7);
        _commentlabel_2.delegate = self;
        _commentlabel_2.enabledTextCheckingTypes = NSTextCheckingTypeLink;
        _commentlabel_2.numberOfLines = 2;
        _commentlabel_2.font = [UIFont systemFontOfSize:14];
//        [_commentlabel_2 sizeToFit];
        
        
    }
    return _commentlabel_2;
}
...
- (UIButton *)commentButton_1 {
    ...
    if (!_commentButton_1) {
        ...
        [_commentButton_1 addTarget:self action:@selector(nameButton_1:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIButton *)commentButton_2 {
    
    if (!_commentButton_2) {
        _commentButton_2 = [[UIButton alloc]init];
        _commentButton_2.backgroundColor = [UIColor clearColor];
        [_commentButton_2 addTarget:self action:@selector(nameButton_2:) forControlEvents:UIControlEventTouchUpInside];
        
    }
    return _commentButton_2;
}


- (void)nameButton_2:(id)sender {
