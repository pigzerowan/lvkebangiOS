...
- (void)createSubViews
{
    ...
    for (int i=0; i<self.confirms.count; i++) {
        ...
        if ([self.confirms[i] isEqualToString:@"继续创建"]) {
            
            [confirm setTitleColor:[UIColor LkbgreenColor] forState:UIControlStateNormal];

        }
        else {
            
            [confirm setTitleColor:CCCUIColorFromHex(0x999999) forState:UIControlStateNormal];
