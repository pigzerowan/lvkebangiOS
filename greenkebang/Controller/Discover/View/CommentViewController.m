...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        UIImage *image =[UIImage imageWithData:[NSData dataWithContentsOfURL:[_userAvatar lkbImageUrl4]]];
        if ([_userAvatar isEqualToString:@"" ] ||_userAvatar == nil) {
            [cell.userImage setImage:YQNormalPlaceImage forState:UIControlStateNormal];
        }
        else {
            [cell.userImage setImage:image forState:UIControlStateNormal];
        }
