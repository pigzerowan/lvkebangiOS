...
- (void)configNewFindGroupTableCellWithGoodModel:(NewFindDetailModel *)admirGood
{
    ...
    [self.headImageView sd_setImageWithURL:[admirGood.groupAvatar lkbImageUrl5] placeholderImage:YQNormalPlaceImage];
    ...
    self.nameLable.text = admirGood.groupName;
    self.adressLable.text= admirGood.groupDesc;
...
{
    if (!_groupBtn) {
        _groupBtn = [[UIButton alloc] initWithFrame:CGRectMake(kDeviceWidth-60, 5, 30, 30)];
        [_groupBtn setImage:[UIImage imageNamed:@"icon_group.png"] forState:UIControlStateNormal];
