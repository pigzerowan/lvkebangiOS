...
- (void)initView {
    self.showImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kDeviceWidth, 480)];
    ...
    CGFloat oriWidth = kDeviceWidth;
    CGFloat oriHeight = self.originalImage.size.height * (oriWidth / self.originalImage.size.width);
    CGFloat oriX = 0 + (self.cropFrame.size.width - oriWidth) / 2;
    CGFloat oriY = 0 + (KDeviceHeight - oriHeight) / 2;
    ...
    self.ratioView.layer.cornerRadius = (kDeviceWidth-40)/2;
...
- (void)initControlBtn {
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 20.0f, 50, 30)];
    cancelBtn.backgroundColor = [UIColor clearColor];
    cancelBtn.titleLabel.textColor = [UIColor whiteColor];
    [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    [cancelBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:14.0f]];
    ...
    UIButton *confirmBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 100.0f, self.view.frame.size.height - 50.0f, 50, 30)];
    confirmBtn.backgroundColor = [UIColor blackColor];
    confirmBtn.titleLabel.textColor = [UIColor whiteColor];
    [confirmBtn setTitle:@"确定" forState:UIControlStateNormal];
    [confirmBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:14.0f]];
    ...
    [confirmBtn setBackgroundColor:[UIColor LkbgreenColor]];
...
//    CGContextStrokePath(ref);
    ...
    CGRect frame = CGRectMake(20, self.ratioView.frame.origin.y, kDeviceWidth-40, kDeviceWidth-40);
    UIBezierPath *circlePath = [UIBezierPath bezierPathWithOvalInRect:frame];
    
    // 设置CAShapeLayer与UIBezierPath关联
    maskLayer.path = circlePath.CGPath;

    path = CGPathCreateMutableCopy(  circlePath.CGPath)   ;
...
- (CGRect)maskRect
{
    ...
    if (UIInterfaceOrientationIsPortrait(self.interfaceOrientation)) {
        diameter = MIN(width, height) - 15 * 2;
    } else {
        diameter = MIN(width, height) - 45 * 2;
    }
